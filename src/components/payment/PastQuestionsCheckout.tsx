...
import type { PastQuestionsProductResponse, PaymentResponse, CourseListItem } from "@/types/api.types";
...
interface PastQuestionsCheckoutProps {
  productCode: string;
  courseIds: string[];
}

export default function PastQuestionsCheckout({ productCode, courseIds }: PastQuestionsCheckoutProps) {
  const t = useTranslations("payment");
  const [product, setProduct] = useState<PastQuestionsProductResponse | null>(null);
  ...
  useEffect(() => {
    Promise.all([STUDENT.getPastQuestionsProducts(), STUDENT.getPastQuestionsCourses()])
      .then(([products, coursesResp]) => {
        const found = products.find((p) => p.code === productCode) ?? null;
        setProduct(found);
        setSubjects(
          found?.subject_count === null
            ? coursesResp.items
            : coursesResp.items.filter((c) => courseIds.includes(c.id))
        );
  ...
  async function handlePay(phone: string) {
    ...
    setErrorText(null);
    try {
      const payment = await STUDENT.initiatePastQuestionsPayment({
  ...
  if (stage === "not-found" || !product) {
    return (
      <div className="mx-auto max-w-md space-y-3 py-16 text-center">
        <p className="text-lg font-bold text-gray-900">{t("unknownPlan")}</p>
        <Link href="/payment/past-questions" className="text-sm font-semibold text-blue-600 hover:underline">
  ...
  const orderItems = subjects.map((s) => ({
    name: s.title.replace(" — Past Questions", ""),
      ...
      <div className="mx-auto max-w-xl space-y-6 rounded-3xl border border-gray-100 bg-white p-6 text-center shadow-md sm:p-8">
        ...
        <div className="space-y-2">
          ...
          <p className="mx-auto max-w-sm text-xs leading-relaxed text-gray-500">
            {isSuccess
              ? `${subjects.length} subject${subjects.length !== 1 ? "s" : ""} unlocked. Happy studying!`
              : t("failedBody")}
          </p>
        </div>
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between border-b border-gray-100 pb-5">
        <div className="flex items-center gap-3">
          <Link href="/payment/past-questions" className="text-gray-400 transition-colors hover:text-gray-600">
